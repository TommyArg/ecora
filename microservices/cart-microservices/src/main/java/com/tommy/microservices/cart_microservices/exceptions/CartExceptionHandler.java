...
@RestControllerAdvice(basePackages = "com.tommy.microservices.cart_microservices")
@Primary
@Slf4j
public class CartExceptionHandler extends GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleFeignException(FeignException exception) {
        var errors = new HashMap<String, String>();
        var fieldName = "Error comunicando con microservicio";

        errors.put(fieldName, exception.getMessage());
        log.warn("Error comunicando con microservicio: {}", exception.toString());
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new ErrorResponse(errors));
    }
}